...
function cli(options) {
  this.options = {
    ...
    , 'string': ['outputDir', 'phantomPath', 'theme', 'css']
  }
...
cli.prototype.parse = function(argv, next) {
  ...
  if (options.help) {
    ...
    ;['outputDir', 'phantomPath', 'theme', 'ganttConfig', 'css'].forEach(function(i) {
    ...
    this.checkPhantom = createCheckPhantom(options.phantomPath, options)
...
function createCheckPhantom(_phantomPath, options) {
  ...
  return function checkPhantom(_next) {
    ...
    if (typeof phantomPath === 'undefined') {
      try {
        ...
        phantomPath = null;
        if (!phantomPath) {
          throw "phantomjs doesnt export path, fallbacking to system paths";
        }        
      } catch (e) {
        options.verbose && console.log(e);
        try {
    ...
    exec(phantomPath + ' --version', function(err, stdout, stderr) {
      if (err) {
        ...
        next(new Error(
            'js-sequence-diagram requires phantomjs '
          + PHANTOM_VERSION
